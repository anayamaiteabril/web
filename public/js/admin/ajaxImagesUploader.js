    ...
    function upload ($form, $imagenUploader) {            
        $form.ajaxForm({
            ...
            success: function(data) {
                ...
                if (data) {
                    $('input[name="image_id"]').val(data.images[0].id);
                    $imagenUploader.find('img')
                        .fadeOut(400, function() {
                            $(this).attr('src',  '/content/' + options.resource + '/thumb/' + data.images[0].src);
                        })
                        .fadeIn(400, function() {
                            //$(this).siblings('a').attr('data-id', data.images[0].id).show();
                        });
                }
            }
        });
